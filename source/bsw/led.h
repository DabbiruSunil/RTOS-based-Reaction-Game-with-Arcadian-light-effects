...
#include "global.h"
    ...
    typedef enum {
        LED_YELLOW,     /**< \brief Identifier for the yellow LED */
        LED_GREEN,
        LED_RED,
        LED_ALL
    } LED_id_t;
    ...
    /**
     * The init function will turn all LED's off
     * \return RC_SUCCESS by default 
     */
    RC_t LED_Init();
    
    /**
     * The set function will turn the selected LED on or off
     * \param LED_id_t id       : IN, will select the LED to be set on or off
     * \param uint8 onOff       : IN, 0 for off, 1 for on
     * \return RC_ERROR_BADPARM or RC_SUCCESS 
     */
    RC_t LED_Set(LED_id_t id, uint8 onOff);
    
    /**
     * The toggle function will toggle the selected LED
     * \param LED_id_t id       : IN, will select the LED to be set on or off
     * \return RC_ERROR_BADPARM or RC_SUCCESS 
     */
    RC_t LED_Toggle(LED_id_t id);
